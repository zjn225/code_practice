...
（比如12 = 4 + 8, 那么这个+1就是指4这个完全平方数，N'就是指这个8），那么这道题本质上就是要求N'是什么时dp[N']是最小的，见如下栗子
...
// ----------------------------------------- 求dp[4]为例，自底向上，比如要求dp[4]，先从dp[1]开始求
i = 1; 1 = 1 + 0       dp[1] = Math.min(dp[1], dp[1 - 1 * 1] + 1) = 1;

i = 2; 2 = 1 + 1       dp[2] = Math.min(dp[2], dp[2 - 1 * 1] + 1) = 2;

i = 3; 3 = 1 + 1 + 1   dp[3] = Math.min(dp[3], dp[3 - 1 * 1] + 1) = 3;

i = 4; 4 = 1 + 1 + 1 + 1  dp[4] = Math.min(dp[4], dp[4 - 1 * 1] + 1) = 4;
i = 4; dp = 2 + 2 dp[4] = Math.min(dp[4], dp[4 - 2 * 2] + 1) = 1; √
